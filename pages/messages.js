...
function Messages() {
  ...
  const [{ messagesResult }, dispatch] = useStateValue();
...

                  {
                    isError ? (<p>Try Again Please</p>)
                      : isLoading ?
                        ...
                        >
                          {(messagesResult.length < 1 ? messages.data.length : messagesResult.length)}
                        </Typography>
                  }
                </Box>
              </Grid>
      ...
      >
        {
          isError ? (<Box display="flex" style={{ margin: 'auto' }}><p>Try Again Please</p></Box>)
            : isLoading ?
              ...
              : messages &&
              (messagesResult.length < 1 ? messages.data : messagesResult)
                .map((user, i) => (
                  <Fragment key={i}>
                    <Box display="flex">
                      <Typography
